...
MCP 클라이언트는 브라우저 세션 쿠키를 쓸 수 없다. 두 가지 길을 둔다.

**1. OAuth 2.1 (기본).** 사용자가 클라이언트에 서버 주소 하나만 넣으면 나머지가 자동으로 진행된다. MCP 인가 명세를 그대로 따른다.
...
클라이언트가 /api/mcp 호출
 ...
 → GET  /oauth/authorize                            (사용자 동의)
 → POST /api/oauth/token                            (PKCE 검증 후 발급)
```

사람이 손대는 지점은 동의 화면의 "연결 승인" 한 번뿐이다. 이 흐름을 붙이기 전에는 토큰 발급 → 브리지 내려받기 → 절대 경로 입력 → 설정 파일 수정 → 앱 재시작의 다섯 단계였고, 어느 하나가 어긋나면 이유를 알 수 없이 실패했다.

지키는 규칙은 넷이다. 넷 다 명세가 MUST로 요구하고, 하나라도 빠지면 방어가 무너진다.

| 규칙 | 없으면 |
|---|---|
| PKCE S256 (plain 거부) | 인가 코드를 엿본 쪽이 그대로 토큰을 받는다 |
| redirect_uri 정확 일치 | 공격자가 코드를 자기 서버로 배달받는다 |
| 인가 코드 1회용 (조건부 UPDATE) | 재사용으로 토큰을 여러 벌 얻는다 |
| 토큰 audience(`resource`) 검증 | 다른 MCP 서버용 토큰이 여기서 통한다 |

`client_id`나 `redirect_uri`가 틀린 인가 요청은 **리다이렉트하지 않고 화면에 세운다**. 그 두 값이 곧 "어디로 보낼지"라, 믿을 수 없는 주소로 오류를 보내면 우리 도메인이 공개 리다이렉터가 된다.

리프레시 토큰은 회전한다. 공개 클라이언트에 대해 명세가 MUST로 요구하는 동작이고, 폐기를 조건부 UPDATE로 해서 같은 토큰이 두 번 쓰이지 않는다.

**2. 정적 접속 토큰 (뒷길).** OAuth를 못 타는 클라이언트와 스크립트를 위해 남긴다. 사용자가 설정 화면에서 발급받아 설정 파일에 넣는다.
...
어느 쪽이든 토큰 원문은 저장하지 않고 해시만 남긴다. 권한은 토큰 소유자의 워크스페이스 권한을 넘지 못한다. 토큰이 유출돼도 그 사용자가 할 수 있는 일 이상은 불가능하다.

사용자는 SET-03에서 승인한 앱 목록을 보고 언제든 끊을 수 있다. 끊을 때 액세스와 리프레시를 함께 폐기한다 — 액세스만 지우면 클라이언트가 리프레시로 곧바로 되살린다.
...
MCP 클라이언트 요청
 ↓
전송 계층 검사 (Origin, 프로토콜 버전)
 ↓
토큰 검증 (해시 대조, 만료·폐기, audience)
 ↓
소유자의 워크스페이스 권한 판정
 ↓
리소스별 권한 필터링
...
전송 계층 검사가 인증보다 **앞**이다. 토큰 조회에 자원을 쓰기 전에 규약 위반을 걸러낸다.

책임:

```
토큰 유효성과 범위 확인
토큰의 audience가 이 서버인지 확인
소유자 권한을 넘지 않는지 확인
리소스 목록을 권한으로 걸러내기
쓰기 도구는 제안만 생성
호출 빈도 제한
