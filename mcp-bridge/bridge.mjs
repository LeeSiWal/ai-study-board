 ...
 *
 * 이 파일 하나만 있으면 된다. 저장소도, 설치도, TypeScript도 필요 없다.
 * Node 18 이상이면 어느 기기에서든 돌아간다. Claude Desktop이 이 서버와
 * 다른 컴퓨터에 있는 것이 보통이라 그렇게 만들었다.
 *
 * Claude Desktop은 원격 MCP 서버에 붙을 때 OAuth 흐름을 탄다. Bearer 토큰을
 * 직접 넣을 자리가 없다. 그 사이를 이 브리지가 메운다. Claude Desktop이
 * 이 파일을 자식 프로세스로 띄우고 stdio로 대화하면, 브리지는 받은 JSON-RPC를
 * 그대로 HTTP 엔드포인트로 넘기고 답을 돌려준다.
 *
 * 프로토콜을 해석하지 않으므로 서버에 도구가 늘어도 이 파일은 그대로다.
 *
 * 설정 예시는 워크스페이스의 설정 → MCP 연결에서 복사할 수 있다.
 */
